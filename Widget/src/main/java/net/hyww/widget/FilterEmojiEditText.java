...
/**
 * 过滤输入法Emoj表情的EditText
 * Created by Xu on 2016/9/28
 */
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (!resetText) {
                    if (count >= 2) {//表情符号的字符长度最小为2
                        CharSequence input = s.subSequence(cursorPos, /*cursorPos + count*/ getSelectionEnd());
                        if (/*containsEmoji(input.toString())*/isEmoji2(input.toString())) {
                            resetText = true;
